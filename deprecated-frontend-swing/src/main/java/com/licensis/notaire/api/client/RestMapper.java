...
public final class RestMapper {
    ...
    private static final GenericRestClient TIPO_IDENT_CLIENT = AdministradorJpa.getInstancia()
    ...
    public static DtoPlantillaTramite toDtoPlantillaTramite(GenericDto dto) {
        ...
        if (tipoDoc != null)
            pt.setTiposDeDocumento(toDtoTipoDeDocumento(tipoDoc));
        return pt;
    }
    ...
    public static DtoDocumentoPresentado toDtoDocumentoPresentado(GenericDto dto) {
        ...
        if (importe instanceof Number n)
            doc.setImporteApagar(n.floatValue());
        Object fechaIng = dto.get("fechaIngreso");
        if (fechaIng instanceof Number n)
            doc.setFechaIngreso(new java.util.Date(n.longValue()));
        Object fechaSal = dto.get("fechaSalida");
        if (fechaSal instanceof Number n)
            doc.setFechaSalida(new java.util.Date(n.longValue()));
        Object fechaVen = dto.get("fechaVencimiento");
        if (fechaVen instanceof Number n)
            doc.setFechaVencimiento(new java.util.Date(n.longValue()));
        Object fechaPag = dto.get("fechaPago");
        if (fechaPag instanceof Number n)
            doc.setFechaPago(new java.util.Date(n.longValue()));
        Object fechaLib = dto.get("fechaLiberado");
        if (fechaLib instanceof Number n)
            doc.setFechaLiberado(new java.util.Date(n.longValue()));
        doc.setQuienEntrega(dto.getString("quienEntrega"));
        GenericDto tramiteDto = asGenericDto(dto.get("fkIdTramite"));
        if (tramiteDto != null)
            doc.setFkTramite(toDtoTramiteMinimal(tramiteDto));
        return doc;
    }
    ...
    public static DtoPersona toDtoPersona(GenericDto dto) {
        ...
        if (tipoIdent != null)
            persona.setDtoTipoIdentificacion(toDtoTipoIdentificacion(tipoIdent));
        return persona;
    }

    public static GenericDto toGenericDto(DtoPersona persona) {
        if (persona == null)
            return null;
        GenericDto dto = new GenericDto();
        if (persona.getIdPersona() != null)
            dto.put("idPersona", persona.getIdPersona());
        dto.put("nombre", persona.getNombre());
        dto.put("apellido", persona.getApellido());
        dto.put("numeroIdentificacion", persona.getNumeroIdentificacion());
        dto.put("telefono", persona.getTelefono());
        dto.put("eMail", persona.getEmail());
        dto.put("domicilio", persona.getDomicilio());
        dto.put("esCliente", persona.getEsCliente());
        dto.put("nacionalidad", persona.getNacionalidad());
        if (persona.getFechaNacimiento() != null)
            dto.put("fechaNacimiento", persona.getFechaNacimiento().getTime());
        dto.put("cuit", persona.getCuit());
        dto.put("estadoCivil", persona.getEstadoCivil());
        dto.put("numeroNupcias", persona.getNumeroNupcias());
        dto.put("sexo", persona.getSexo());
        dto.put("ocupacion", persona.getOcupacion());
        dto.put("registroEscribano", persona.getRegistroEscribano());
        if (persona.getDtoTipoIdentificacion() != null) {
            GenericDto tDto = new GenericDto();
            tDto.put("idTipoIdentificacion", persona.getDtoTipoIdentificacion().getIdTipoIdentificacion());
            dto.put("fkIdTipoIdentificacion", tDto);
        }
        return dto;
    }
    ...
    public static DtoGestionDeEscritura toDtoGestion(GenericDto dto) {
        ...
        else if (fecha instanceof Number n)
            gestion.setFechaInicio(new Date(n.longValue()));
        GenericDto estado = asGenericDto(dto.get("fkIdEstadoDeGestion"));
        if (estado != null)
            gestion.setEstado(toDtoEstado(estado));
        ...
        if (cliente != null)
            gestion.setClienteReferencia(toDtoPersona(cliente));
        return gestion;
    }

    /**
     * Maps Presupuesto API response to DtoPresupuesto (avoids circular ref with
     * tramite).
     */
    public static DtoPresupuesto toDtoPresupuesto(GenericDto dto) {
        ...
        if (fecha instanceof Date date)
            ...
            pres.setFecha(new Date(n.longValue()));
        GenericDto personaDto = asGenericDto(dto.get("fkIdPersona"));
        if (personaDto != null)
            pres.setPersona(toDtoPersona(personaDto));
        GenericDto tramiteDto = asGenericDto(dto.get("fkIdTramite"));
        if (tramiteDto != null) {
            ...
            if (gestion != null)
                tramite.setGestionDeEscritura(toDtoGestion(gestion));
    ...
    public static DtoTramite toDtoTramite(GenericDto dto) {
        ...
        if (presupuesto != null)
            tramite.setPresupuesto(toDtoPresupuesto(presupuesto));
        GenericDto tipoTramite = asGenericDto(dto.get("fkIdTipoTramite"));
        if (tipoTramite != null)
            tramite.setTiposDeTramite(toDtoTipoTramite(tipoTramite));
        GenericDto gestion = asGenericDto(dto.get("fkIdGestion"));
    ...
    public static DtoHistorial toDtoHistorial(GenericDto dto) {
        ...
        if (fecha instanceof Date date)
            ...
            hist.setFecha(new Date(n.longValue()));
        GenericDto estado = asGenericDto(dto.get("fkIdEstadoDeGestion"));
        if (estado != null)
            hist.setEstadosDeGestion(toDtoEstado(estado));
        return hist;
    }
