...
public class DarAltaEscribano extends javax.swing.JInternalFrame
{
    ...
    {//GEN-HEADEREND:event_botonAceptarActionPerformed
        ...
        try
        {
            ...
            if (dtoNuevoEscribano.isValido())
            {
                try
                {
                    // Convertir DtoPersona a GenericDto
                    GenericDto personaDto = convertirDtoPersonaAGenericDto(dtoNuevoEscribano);
                    
                    if (labelTitulo.getText().contains(ConstantesGui.DAR_ALTA_ESCRIBANO))
                    {
                        // Crear nueva persona (escribano)
                        personaClient.create(personaDto);
                        JOptionPane.showMessageDialog(this, "Se ha " + labelTitulo.getText() + ": " + dtoNuevoEscribano.getNombre());
                        this.salir();
                    }
                    else if (labelTitulo.getText().contains(ConstantesGui.MODIFICAR_ESCRIBANO))
                    {
                        // Actualizar persona existente
                        personaClient.edit(personaDto);
                        JOptionPane.showMessageDialog(this, "Se ha " + labelTitulo.getText() + ": " + dtoNuevoEscribano.getNombre());
                        this.salir();
                    }
                }
                catch (IOException ex)
                {
                    ...
                    logger.log(Level.SEVERE, "Error al dar de alta/modificar escribano", ex);
                }
    ...
    /**
     * Convierte DtoPersona a GenericDto para uso con REST API
     */
    private GenericDto convertirDtoPersonaAGenericDto(DtoPersona dtoPersona)
    {
        GenericDto genericDto = new GenericDto();
        
        if (dtoPersona.getIdPersona() != null)
        {
            genericDto.put("idPersona", dtoPersona.getIdPersona());
        }
        genericDto.put("nombre", dtoPersona.getNombre());
        genericDto.put("apellido", dtoPersona.getApellido());
        genericDto.put("nacionalidad", dtoPersona.getNacionalidad());
        genericDto.put("cuit", dtoPersona.getCuit());
        genericDto.put("sexo", dtoPersona.getSexo());
        genericDto.put("fechaNacimiento", dtoPersona.getFechaNacimiento());
        genericDto.put("estadoCivil", dtoPersona.getEstadoCivil());
        genericDto.put("numeroNupcias", dtoPersona.getNumeroNupcias());
        genericDto.put("ocupacion", dtoPersona.getOcupacion());
        genericDto.put("domicilio", dtoPersona.getDomicilio());
        genericDto.put("telefono", dtoPersona.getTelefono());
        genericDto.put("registroEscribano", dtoPersona.getRegistroEscribano());
        genericDto.put("esCliente", dtoPersona.isEsCliente());
        genericDto.put("numeroidentificacion", dtoPersona.getNumeroIdentificacion());
        genericDto.put("eMail", dtoPersona.getEmail());
        
        if (dtoPersona.getDtoTipoIdentificacion() != null)
        {
            // Necesitamos el ID del tipo de identificación
            // Por ahora, si hay un ID, lo agregamos
            if (dtoPersona.getDtoTipoIdentificacion().getIdTipoIdentificacion() != null)
            {
                genericDto.put("fkIdTipoIdentificacion", dtoPersona.getDtoTipoIdentificacion().getIdTipoIdentificacion());
            }
        }
        
        if (dtoPersona.getVersion() != null)
        {
            genericDto.put("version", dtoPersona.getVersion());
        }
