...
class AuditAspectTest {
    ...
    @BeforeEach
    void setUp() {
        aspect = new AuditAspect(auditService, userRepository);
        adminUser = new User();
    ...
    private void bindRequestWithUserHeader(String username) {
        ...
        if (username != null) {
            request.addHeader(AuditAspect.ACTING_USER_HEADER, username);
        }
        RequestContextHolder.setRequestAttributes(new ServletRequestAttributes(request));
    }

    private Method deedMethod(String name, Class<?>... params) throws NoSuchMethodException {
    ...
    void shouldPersistAuditRecordWhenAuthenticatedUserPerformsMutation() throws NoSuchMethodException {
        ...
        assertThat(saved.getModule()).isEqualTo("Deeds");
        assertThat(saved.getFkIdUser()).isSameAs(adminUser);
        assertThat(saved.getDate()).isNotNull();
        assertThat(saved.getOperationDetail()).isNotBlank();
    }
    ...
    void shouldSkipReadOnlyGetOperations() throws NoSuchMethodException {
        authenticateAs("admin");
        Method method = deedMethod("getById", Integer.class);
        stubJoinPoint(DeedController.class, method, new Object[]{1});
    ...
    void shouldSwallowAuditFailuresWithoutThrowing() throws NoSuchMethodException {
        authenticateAs("admin");
        when(userRepository.findAll()).thenReturn(List.of(adminUser));
        lenient().when(auditService.save(any()))
                .thenThrow(new RuntimeException("simulated DB failure"));

        Method method = deedMethod("delete", Integer.class);
        stubJoinPoint(DeedController.class, method, new Object[]{1});
