...
class AuditFlowIntegrationTest {
    ...
    void setUp() {
        ...
        Optional<User> admin = userRepository.findAll().stream()
                .filter(u -> "admin".equalsIgnoreCase(u.getName()))
                .findFirst();
        admin.ifPresent(user -> SecurityContextHolder.getContext().setAuthentication(
    ...
    void shouldWriteAuditRowWhenAuthenticatedHitController() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/estado-gestion")
                        ...
                        .content("{\"name\":\"Audit IT\",\"notes\":\"audit flow\"}"))
                .andExpect(status().isCreated());

        long after = auditRepository.count();
        assertThat(after).isGreaterThan(before);

        // The latest audit row must describe a "ManagementStatuses" creation.
        boolean hasMatch = auditRepository.findAll().stream()
                .anyMatch(r -> "ManagementStatuses".equals(r.getModule())
                        && r.getOperationDetail() != null
                        && r.getOperationDetail().toLowerCase().contains("creation"));
    ...
    void shouldExposeAuditRecordsViaPaginatedDtoEndpoint() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/audit-log?page=0&size=5&sort=date,desc"))
                .andExpect(status().isOk())
    ...
    @Test
    @DisplayName("Should support filtering audit records by module")
    void shouldFilterAuditRecordsByModule() throws Exception {
        mockMvc.perform(get("/api/v1/conceptos")).andExpect(status().isOk());

        mockMvc.perform(get("/api/v1/audit-log?page=0&size=10&module=Conceptos"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content").isArray());
    }
}
