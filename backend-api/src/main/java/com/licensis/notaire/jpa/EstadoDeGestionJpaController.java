...
public class EstadoDeGestionJpaController implements Serializable, IPersistenciaJpa {
    ...
    public Integer create(EstadoDeGestion estadoDeGestion) throws PreexistingEntityException {
        ...
        if (estadoDeGestion.getHistorialList() == null) {
            estadoDeGestion.setHistorialList(new java.util.HashSet<Historial>());
        }
        EntityManager em = null;
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            java.util.Set<Historial> attachedHistorialList = new java.util.HashSet<Historial>();
            for (Historial historialListHistorialToAttach : estadoDeGestion.getHistorialList()) {
                historialListHistorialToAttach = em.getReference(historialListHistorialToAttach.getClass(),
                        historialListHistorialToAttach.getIdHistorial());
            ...
            for (Historial historialListHistorial : estadoDeGestion.getHistorialList()) {
                EstadoDeGestion oldFkIdEstadoGestionOfHistorialListHistorial = historialListHistorial
                        .getFkIdEstadoGestion();
                ...
                if (oldFkIdEstadoGestionOfHistorialListHistorial != null) {
                    ...
                    oldFkIdEstadoGestionOfHistorialListHistorial = em
                            .merge(oldFkIdEstadoGestionOfHistorialListHistorial);
                }
            }
    ...
    public Boolean edit(EstadoDeGestion estadoDeGestion) throws IllegalOrphanException, NonexistentEntityException,
        ...
        em = getEntityManager();
        em.getTransaction().begin();
        EstadoDeGestion persistentEstadoDeGestion = em.find(EstadoDeGestion.class,
        ...
        if (persistentEstadoDeGestion != null) {
            ...
            if (version != oldVersion) // Si son distintas "Alguien modifico o elimino el objeto"
            {
                ...
                java.util.Set<Historial> historialListOld = persistentEstadoDeGestion.getHistorialList();
                java.util.Set<Historial> historialListNew = estadoDeGestion.getHistorialList();
                ...
                for (Historial historialListOldHistorial : historialListOld) {
                    if (!historialListNew.contains(historialListOldHistorial)) {
                        ...
                        illegalOrphanMessages.add("You must retain Historial " + historialListOldHistorial
                ...
                java.util.Set<Historial> attachedHistorialListNew = new java.util.HashSet<Historial>();
                for (Historial historialListNewHistorialToAttach : historialListNew) {
                    historialListNewHistorialToAttach = em.getReference(historialListNewHistorialToAttach.getClass(),
                            historialListNewHistorialToAttach.getIdHistorial());
                ...
                for (Historial historialListNewHistorial : historialListNew) {
                    if (!historialListOld.contains(historialListNewHistorial)) {
                        EstadoDeGestion oldFkIdEstadoGestionOfHistorialListNewHistorial = historialListNewHistorial
                                .getFkIdEstadoGestion();
                        ...
                        if (oldFkIdEstadoGestionOfHistorialListNewHistorial != null
                                && !oldFkIdEstadoGestionOfHistorialListNewHistorial.equals(estadoDeGestion)) {
                            oldFkIdEstadoGestionOfHistorialListNewHistorial.getHistorialList()
                                    .remove(historialListNewHistorial);
                            oldFkIdEstadoGestionOfHistorialListNewHistorial = em
                                    .merge(oldFkIdEstadoGestionOfHistorialListNewHistorial);
                        }
                    }
                }
                em.getTransaction().commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            List<String> illegalOrphanMessages = null;
            java.util.Set<Historial> historialListOrphanCheck = estadoDeGestion.getHistorialList();
            for (Historial historialListOrphanCheckHistorial : historialListOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This EstadoDeGestion (" + estadoDeGestion
                        + ") cannot be destroyed since the Historial " + historialListOrphanCheckHistorial
