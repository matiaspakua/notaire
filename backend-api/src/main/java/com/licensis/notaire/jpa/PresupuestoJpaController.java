...
public class PresupuestoJpaController implements Serializable, IPersistenciaJpa {
    ...
    public int create(Presupuesto presupuesto) {
        ...
        if (presupuesto.getPagoList() == null) {
            presupuesto.setPagoList(new java.util.HashSet<Pago>());
        }
        ...
        try {
            ...
            for (Pago pagoListPagoToAttach : presupuesto.getPagoList()) {
                pagoListPagoToAttach = em.getReference(pagoListPagoToAttach.getClass(),
                        pagoListPagoToAttach.getIdPago());
            ...
            for (Tramite tramiteListTramiteToAttach : presupuesto.getTramiteList()) {
                tramiteListTramiteToAttach = em.getReference(tramiteListTramiteToAttach.getClass(),
                        tramiteListTramiteToAttach.getIdTramite());
    ...
    public Boolean edit(Presupuesto presupuesto)
            ...
            if (version != oldVersion) // Si son distintas "Alguien modifico el objeto"
            {
                ...
                for (Pago pagoListOldPago : pagoListOld) {
                    if (!pagoListNew.contains(pagoListOldPago)) {
                        ...
                        illegalOrphanMessages.add("You must retain Pago " + pagoListOldPago
                ...
                for (Tramite tramiteListOldTramite : tramiteListOld) {
                    if (!tramiteListNew.contains(tramiteListOldTramite)) {
                        ...
                        illegalOrphanMessages.add("You must retain Tramite " + tramiteListOldTramite
                                + " since its fkIdPresupuesto field is not nullable.");
                    }
                }
                ...
                for (Pago pagoListNewPagoToAttach : pagoListNew) {
                    pagoListNewPagoToAttach = em.getReference(pagoListNewPagoToAttach.getClass(),
                            pagoListNewPagoToAttach.getIdPago());
                ...
                for (Tramite tramiteListNewTramiteToAttach : tramiteListNew) {
                    tramiteListNewTramiteToAttach = em.getReference(tramiteListNewTramiteToAttach.getClass(),
                            tramiteListNewTramiteToAttach.getIdTramite());
                ...
                for (Pago pagoListNewPago : pagoListNew) {
                    if (!pagoListOld.contains(pagoListNewPago)) {
                        ...
                        if (oldFkIdPresupuestoOfPagoListNewPago != null
                                && !oldFkIdPresupuestoOfPagoListNewPago.equals(presupuesto)) {
                ...
                for (Tramite tramiteListNewTramite : tramiteListNew) {
                    if (!tramiteListOld.contains(tramiteListNewTramite)) {
                        Presupuesto oldFkIdPresupuestoOfTramiteListNewTramite = tramiteListNewTramite
                                .getFkIdPresupuesto();
                        tramiteListNewTramite.setFkIdPresupuesto(presupuesto);
                        tramiteListNewTramite = em.merge(tramiteListNewTramite);
                        if (oldFkIdPresupuestoOfTramiteListNewTramite != null
                                && !oldFkIdPresupuestoOfTramiteListNewTramite.equals(presupuesto)) {
                            oldFkIdPresupuestoOfTramiteListNewTramite.getTramiteList().remove(tramiteListNewTramite);
                            oldFkIdPresupuestoOfTramiteListNewTramite = em
                                    .merge(oldFkIdPresupuestoOfTramiteListNewTramite);
                        }
                    }
                }
                for (Item itemListNewItem : itemListNew) {
                    if (!itemListOld.contains(itemListNewItem)) {
                        ...
                        if (oldFkIdPresupuestoOfItemListNewItem != null
                                && !oldFkIdPresupuestoOfItemListNewItem.equals(presupuesto)) {
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            List<String> illegalOrphanMessages = null;
            java.util.Set<Pago> pagoListOrphanCheck = presupuesto.getPagoList();
            for (Pago pagoListOrphanCheckPago : pagoListOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This Presupuesto (" + presupuesto + ") cannot be destroyed since the Pago "
                        + pagoListOrphanCheckPago + " in its pagoList field has a non-nullable fkIdPresupuesto field.");
            }
            ...
            for (Tramite tramiteListOrphanCheckTramite : tramiteListOrphanCheck) {
                ...
                illegalOrphanMessages.add("This Presupuesto (" + presupuesto
                        + ") cannot be destroyed since the Tramite " + tramiteListOrphanCheckTramite
            ...
            for (Item itemListOrphanCheckItem : itemListOrphanCheck) {
                ...
                illegalOrphanMessages.add("This Presupuesto (" + presupuesto + ") cannot be destroyed since the Item "
