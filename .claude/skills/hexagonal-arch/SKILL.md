...
- **Lazy-loaded associations serialized past the transaction boundary**: an outbound adapter method (JPA-backed) that returns an entity with `FetchType.LAZY` associations will throw when the *inbound* adapter (e.g. a `@Transactional(readOnly = true)` REST controller) tries to serialize it — the transaction closes when the controller method returns, before Jackson walks the response body. Fixing the detail-lookup query (`findById`) with a `JOIN FETCH` is not enough on its own — check every other outbound port method that can return the same entity type to a serializing adapter (`findAll`, `findByX`, etc.), since each one hits the same failure mode independently and H2-based unit/integration tests often don't catch it (no real lazy proxy behavior without a populated Postgres row).
